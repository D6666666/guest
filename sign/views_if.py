...
def add_guest(request):
    ...
    event_limit = Event.objects.get(id=eid).limit  #发布会人数限制
    guest_limit = Guest.objects.filter(event_id=eid)  #已添加的嘉宾
...
def get_guest_list(request):
    ...
    if eid != '' and phone != '':
        ...
        try:
            ...
            return JsonResponse({"stauts": 200, 'message': 'success', 'data': guest})
...
def user_sign(request):
    ...
    if result:
        return JsonResponse({'status': 10027, 'message': 'user hsa sign in'})
    else:
        Guest.objects.create(event_id=eid, phone=phone).update(sign='1')
